...
public class Main {
    public static void main(String[] args) {
        ...
        for (Studio i : studioDatabase) {
            int c = 0;
            for (Film j : i.getFilm())
                c++;
            if (c > 2)
                System.out.println("Studiourile care au publicat mai multe de 2 filme sunt " + (i.getNume()));
        }
        for (Studio i : studioDatabase){
            for (Film j : i.getFilm()){
                for (Actor k: j.getActori()){
                   if( k.getNume() == "actor cu 2 oscaruri")
                       System.out.println("Nume studioului in care joaca actorul cu doua oscaruri este "+ i.getNume());
                }
            }
        }
        System.out.print("Numele filmelor in care actorul are peste 50 de ani sunt");
        for (Studio i : studioDatabase){
            for (Film j : i.getFilm()){
                for (Actor k : j.getActori()){
                    if (k.getVarsta() > 50)
